...
\item deploy proper derivatives for location parameters, in particular epicenter, in either a local or global \textit{cartesian} coordinate system (instead of $lat$ and $lon$). \blue{Note a key modification from the old} \verb+INVERSION.PAR+\blue{: $dlat/lon$ now becomes $dX_E/X_N$}.
