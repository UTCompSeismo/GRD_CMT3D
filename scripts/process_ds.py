...
if preprocess:
  ...
  if regional_data:
    error1=os.system('process_cal_data.pl -m '+cmt+' -p -s '+sps+' '+' '.join(data_list)+'>/dev/null')
  else:
    error1=os.system('process_data_new.pl -m '+cmt+' -p -s '+sps+' '+' '.join(data_list)+'>/dev/null')
#  print 'process_data_new.pl -m '+cmt+' -p -s '+sps+' '+' '.join(data_list)
  if error1 != 0:
...
#  error1=os.system('process_data_new.pl -R '+resp_dir+' -d '+new_datadir+' -t '+tmin+'/'+tmax+' '+cdata_list+'>/dev/null ')
  if regional_data:
    error1=os.system('process_cal_data.pl -i none -d '+new_datadir+' -t '+tmin+'/'+tmax+' '+cdata_list+'>/dev/null')
  else:
...
if windowing:
  ...
  for i in range(0,nsta):
    ...
    if regional_data:
      new_data=glob.glob(new_datadir+'/*'+net+'.'+sta+'.'+cmp+'*'+dataext+'.c')
    else:
